...
PedidoModel.findById = (id, result) => {
  sql.query("SELECT * FROM pedidos WHERE idpedido = " + id, (err, res) => {
    if (err) {
      console.log("erro: ", err);
      result(null, err);
      return;
...
PedidoModel.updateById = (id, pedido, result) => {
  sql.query(
    "UPDATE pedidos SET hora = ?, status = ? WHERE idpedido = ? ",
    [pedido.hora, pedido.status, id],
    (err, res) => {
      if (err) {
        console.log("erro: ", err);
        result(null, err);
...
PedidoModel.remove = (id, result) => {
  sql.query("DELETE FROM pedidos WHERE idpedido = ?", id, (err, res) => {
    if (err) {
      console.log("erro: ", err);
      result(err, null);
    } else if (res.affectedRows == 0) {
